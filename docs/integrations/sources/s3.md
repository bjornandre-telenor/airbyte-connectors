...
This page contains the setup guide and reference information for the [S3](https://aws.amazon.com/s3/) source connector.
...
- **Skip Full Check for Parquet**: When enabled, the connection test verifies that Parquet files are accessible but skips the full record-parse step. Enable this option if you encounter out-of-memory errors during the connection test on large Parquet files. Disabled by default.
